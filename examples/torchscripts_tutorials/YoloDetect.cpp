...
bool YoloDetection::Detect(cv::Mat mRGB)
{
    ...
    torch::Tensor preds = mModule.forward({imgTensor}).toTensor();
    ...
    if (dets.size() > 0)
    {
        ...
        for (size_t i=0; i < dets[0].sizes()[0]; ++ i)
        {
            ...
            cv::Point bottom_right(bottom, right);
            cv::rectangle(mRGB, top_left, bottom_right, cv::Scalar(0, 255, 0), 2);

            // Label
            std::string label = "ID: " + std::to_string(classID);
            cv::putText(mRGB, label, top_left, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
        }
        
    }

    // Show image with bounding boxes
    cv::imshow("Detections", mRGB);
    cv::waitKey(0);
    return true;
}
