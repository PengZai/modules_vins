...
void PoseEstimator::pipeline(CameraFrame &camera_frame){
    ...
    if(this->camera_frame_deque_.size()>1){
        ...
        for(int i=0; i < (int)img_0_from_current_frame->matches_in_time_.size();i++){
            ...
            double z = kp_from_previous_frame->pt3d_.z;
            ...
            reprojected_pt2is.push_back(camera2pixel(kp_from_previous_frame->pt3d_, cv_K));
            prev_pt2ds.push_back(kp_from_previous_frame->pt2i_);
        ...
        if(checkEstimatedPose(estimated_T_current_cam_previous_cam, inlier_num) == true){
