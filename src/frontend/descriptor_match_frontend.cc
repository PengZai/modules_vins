...
void DescriptorMatchFrontend::initPipeline(const std::shared_ptr<CameraFrame> &camera_frame){
...
void DescriptorMatchFrontend::normalPipeline(const std::shared_ptr<CameraFrame> &camera_frame){
    ...
    for(size_t i=0; i<this->ref_camera_frame_deque_.size();i++){
        ...
        if(camera_frame->status_ == CameraFrame::Status::NORMAL){
            ...
            LOG(INFO) << YELLOW << " reference camera frame time stamp: " << std::to_string(ref_camera_frame->image_vector_.at(0)->timestamp_) << " camera frame time stamp : " << std::to_string(camera_frame->image_vector_.at(0)->timestamp_) << RESET;
