...
class State{
    ...
    public:
        ...
        bool getPoseComparisonForTbwWithIdx(const double base_timestamp, size_t used_idx, Sophus::SE3d &T_b_w);
        ...
        std::vector<std::map<double, Sophus::SE3<double>>> timestamp_pose_comparison_in_comparison_full_map_vector_; // pose of comparison but take the comparison as base
        std::vector<std::map<double, Sophus::SE3<double>>> timestamp_pose_comparison_in_base_map_vector_; // a sub map of comparison pose in world coordinate

     
        std::shared_ptr<SystemConfig> sys_config_;
        std::vector<Sophus::SE3<double>> T_base_comparison_vector_;
