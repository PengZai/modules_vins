...
std::vector<float> YOLOSegmentor::Letterbox(const cv::Mat& src, cv::Mat& dst, const cv::Size& out_size) {
...
cv::Rect YOLOSegmentor::toBox(const cv::Mat& input, const cv::Rect& range){
...
void draw_result(cv::Mat& image, std::vector<SegmentOutput>& results)
{
    ...
    for (const SegmentOutput& result : results) {
        cv::rectangle(image, result.box_, cv::Scalar(0, 255, 0), 2, 8);
        mask(result.box_).setTo(cv::Scalar(0, 0, 255), result.boxMask_);
    }
...
void YOLOSegmentor::detect(const std::shared_ptr<Image> &img){
    ...
    // torch::Tensor image_tensor = torch::from_blob(resize_image.data, { resize_image.rows, resize_image.cols, 3 }, torch::kByte);
    image_tensor = image_tensor.toType(torch::kFloat32).div(255);
    image_tensor = image_tensor.permute({ 2, 0, 1 });
    image_tensor = image_tensor.unsqueeze(0);

    std::vector<torch::jit::IValue> inputs{ image_tensor };
    auto net_outputs = this->model_.forward(inputs).toTuple();
    ...
    for (int i = 0; i < detect_buffer.rows; ++i) {
        ...
        if (score > this->conf_threshold_) {
            ...
            cv::Mat detection_box = result.colRange(0, 4);

            cv::Rect mask_box = toBox(detection_box * mask_scale,
                cv::Rect(0, 0, 160, 160));
            
            // scale detected box into original size
            detection_box.at<float>(0)-= pad_info[0];
            detection_box.at<float>(1)-= pad_info[1];
            detection_box /= pad_info[2];


            cv::Rect image_box = toBox(detection_box,
    ...
    cv::dnn::NMSBoxes(boxes, confidences, this->conf_threshold_, this->nms_threshold_, nms_indexes);

    for (const int index : nms_indexes) {
        SegmentOutput segmentOutput;
        segmentOutput.class_id_ = class_ids[index];
        segmentOutput.confidence_ = confidences[index];
        segmentOutput.box_ = boxes[index];
        // sigmoid运算
        cv::Mat m;
        ...
        cv::Mat block_m = m(mask_boxes[index]) > 0.5f;

        cv::resize(m(mask_boxes[index]) > 0.5f,
        segmentOutput.boxMask_, segmentOutput.box_.size());

        img->segment_outputs_.emplace_back(segmentOutput);
    }

    // draw_result(resize_image, img->segment_outputs_);
