...
model_path_(model_path)
{
    ...
    this->model_ = torch::jit::load(model_path_);
    
    this->model_.eval();
    this->model_.to(this->device_, torch::kFloat32);

}



float YOLODetector::generate_scale(cv::Mat& image, const std::vector<int>& target_size) {
...
float YOLODetector::letterbox(cv::Mat &input_image, cv::Mat &output_image, const std::vector<int> &target_size) {
...
torch::Tensor YOLODetector::xyxy2xywh(const torch::Tensor& x) {
...
torch::Tensor YOLODetector::nms(const torch::Tensor& bboxes, const torch::Tensor& scores, float iou_threshold) {
    if (bboxes.numel() == 0)
...
torch::Tensor YOLODetector::non_max_suppression(torch::Tensor& prediction, float conf_thres, float iou_thres, int max_det) {
...
torch::Tensor YOLODetector::clip_boxes(torch::Tensor& boxes, const std::vector<int>& shape) {
...
torch::Tensor YOLODetector::scale_boxes(const std::vector<int>& img1_shape, torch::Tensor& boxes, const std::vector<int>& img0_shape) {
...
void YOLODetector::detect(const std::shared_ptr<Image> &img){

    // Load image and preprocess
    cv::Mat image = img->color_data_;
    cv::Mat input_image;
    letterbox(img->color_data_, input_image, {640, 640});
    cv::cvtColor(input_image, input_image, cv::COLOR_BGR2RGB);

    torch::Tensor image_tensor = torch::from_blob(input_image.data, {input_image.rows, input_image.cols, 3}, torch::kByte).to(this->device_);
    ...
    // Inference
    torch::Tensor output = this->model_.forward(inputs).toTensor().cpu();

    // NMS
    torch::Tensor detected_boxes = non_max_suppression(output)[0];
    torch::Tensor boxes = detected_boxes.index({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, 4)});
    detected_boxes.index_put_({torch::indexing::Slice(), torch::indexing::Slice(torch::indexing::None, 4)}, scale_boxes({input_image.rows, input_image.cols}, boxes, {image.rows, image.cols}));
    

    for(int j = 0; j < detected_boxes.size(0); j++) {

        BoxOutput box_output;

        int x1 = detected_boxes[j][0].item().toFloat();
        int y1 = detected_boxes[j][1].item().toFloat();
        int x2 = detected_boxes[j][2].item().toFloat();
        int y2 = detected_boxes[j][3].item().toFloat();
        float conf = detected_boxes[j][4].item().toFloat();
        int cls = detected_boxes[j][5].item().toInt();

        box_output.class_id_ = cls;
        box_output.confidence_ = conf;
        box_output.box_ = cv::Rect(x1, y1, x2-x1, y2-y1);
        img->box_outputs_.emplace_back(box_output);
    }
